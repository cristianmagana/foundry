...
import {
    TeamPermissionConfig,
    EnvironmentConfig,
    EnvironmentVariables,
    BranchProtectionPreset,
    RepositorySecret,
} from '../api/types/productionalization';
 ...
 */
export const parseTeamPermissions = (input: string): TeamPermissionConfig[] => {
    ...
    try {
        ...
        return parsed.map((item, index) => {
            ...
            if (!validPermissions.includes(item.permission)) {
                throw new Error(
                    `Team permission at index ${index} has invalid permission: ${item.permission}. Must be one of: ${validPermissions.join(', ')}`
                );
            }
        ...
        throw new Error(
            `Failed to parse team permissions: ${error instanceof Error ? error.message : String(error)}`
        );
    }
};
 ...
 */
export const parseTopics = (input: string): string[] => {
    if (!input || input.trim() === '') {
        return [];
    }
 ...
 * @throws Error if JSON is invalid
 */
export const parseEnvironments = (input: string): EnvironmentConfig[] => {
    if (!input || input.trim() === '' || input.trim() === '[]') {
        return [];
    }
    ...
    try {
        ...
        return parsed.map((item, index) => {
            ...
            if (item.waitTimer !== undefined) {
                if (typeof item.waitTimer !== 'number' || item.waitTimer < 0) {
                    throw new Error(
                        `Environment ${item.name} has invalid waitTimer (must be a non-negative number)`
                    );
                }
            ...
            if (item.reviewers !== undefined) {
                ...
                config.reviewers = item.reviewers.map((reviewer: unknown, rIndex: number) => {
                    if (!reviewer || typeof reviewer !== 'object') {
                        throw new Error(
                            `Environment ${item.name} reviewer at index ${rIndex} must be an object`
                        );
                    }
                    const rev = reviewer as Record<string, unknown>;
                    if (!rev.type || !['User', 'Team'].includes(rev.type as string)) {
                        throw new Error(
                            `Environment ${item.name} reviewer at index ${rIndex} has invalid type (must be 'User' or 'Team')`
                        );
                    }
                    if (!rev.slug || typeof rev.slug !== 'string') {
                        throw new Error(
                            `Environment ${item.name} reviewer at index ${rIndex} missing valid slug`
                        );
                    }

                    return {
                        type: rev.type as 'User' | 'Team',
                        slug: rev.slug,
                    };
                });
            }
        ...
        throw new Error(
            `Failed to parse environments: ${error instanceof Error ? error.message : String(error)}`
        );
    }
};
 ...
 * @throws Error if JSON is invalid
 */
export const parseEnvironmentVariables = (input: string): EnvironmentVariables[] => {
    if (!input || input.trim() === '' || input.trim() === '[]') {
        return [];
    }
    ...
    try {
        ...
        return parsed.map((item, index) => {
            if (!item.environmentName || typeof item.environmentName !== 'string') {
                throw new Error(
                    `Environment variables at index ${index} missing valid environmentName`
                );
            }

            if (!item.variables || !Array.isArray(item.variables)) {
                throw new Error(
                    `Environment variables for ${item.environmentName} must contain a variables array`
                );
            }

            const variables = item.variables.map((variable: unknown, vIndex: number) => {
                if (!variable || typeof variable !== 'object') {
                    throw new Error(
                        `Variable at index ${vIndex} for environment ${item.environmentName} must be an object`
                    );
                }
                const v = variable as Record<string, unknown>;
                if (!v.name || typeof v.name !== 'string') {
                    throw new Error(
                        `Variable at index ${vIndex} for environment ${item.environmentName} missing valid name`
                    );
                }
                if (v.value === undefined || typeof v.value !== 'string') {
                    throw new Error(
                        `Variable ${v.name} for environment ${item.environmentName} missing valid value`
                    );
                }
        ...
        throw new Error(
            `Failed to parse environment variables: ${error instanceof Error ? error.message : String(error)}`
        );
    }
};
 ...
 */
export const parseBranchProtectionPreset = (input: string): BranchProtectionPreset | undefined => {
    if (!input || input.trim() === '') {
        return undefined;
    }

    const validPresets: BranchProtectionPreset[] = ['strict', 'moderate', 'minimal'];
    const preset = input.trim() as BranchProtectionPreset;

    if (!validPresets.includes(preset)) {
        throw new Error(
            `Invalid branch protection preset: ${input}. Must be one of: ${validPresets.join(', ')}`
        );
    }
 ...
 * @throws Error if JSON is invalid
 */
export const parseSecrets = (input: string): RepositorySecret[] => {
    if (!input || input.trim() === '' || input.trim() === '[]') {
        return [];
    }
    ...
    try {
        ...
        throw new Error(
            `Failed to parse repository secrets: ${error instanceof Error ? error.message : String(error)}`
        );
    }
};
