...
export const getRepositoryService = (client: Octokit, log: Logger): RepositoryService => {
    const updateDefaultBranch = async (fullName: string, defaultBranch: string): Promise<void> => {
        ...
        try {
            ...
            try {
                ...
                log.info(
                    `Successfully renamed branch from ${currentDefaultBranch} to ${defaultBranch}`
                );
            } catch (error) {
                // If rename fails (e.g., branch doesn't exist), just update the default branch setting
                log.warn(
                    `Could not rename branch: ${error}. Attempting to update default branch setting.`
                );
