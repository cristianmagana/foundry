...
import {
    ProductionalizationConfig,
    ProductionalizationResult,
    ...
    EnvironmentReviewer,
} from '../types/productionalization';
import {toUpperSnakeCase} from '../../util/caseConverter';
import {encryptSecret} from '../../util/secretEncryption';

/**
 * Productionalization service interface
 */
export interface ProductionalizationService {
    productionalizeRepository: (
        owner: string,
        repo: string,
        config: ProductionalizationConfig
    ) => Promise<ProductionalizationResult>;
}
 ...
 * @returns Team ID
 */
const resolveTeamSlugToId = async (
    client: Octokit,
    log: Logger,
    org: string,
    teamSlug: string
    ...
    try {
        ...
        throw new Error(
            `Failed to resolve team slug '${teamSlug}': ${error instanceof Error ? error.message : String(error)}`
        );
    }
};
...
const resolveUsernameToId = async (
    ...
    try {
        ...
        throw new Error(
            `Failed to resolve username '${username}': ${error instanceof Error ? error.message : String(error)}`
        );
    }
};
 ...
 */
const addTeamPermissions = async (
    client: Octokit,
    log: Logger,
    owner: string,
    repo: string,
    teams: TeamPermissionConfig[]
...
const createEnvironments = async (
    ...
    created: string[];
    errors: EnvironmentCreationResult[];
}> => {
    log.info(`Creating environments for ${owner}/${repo}`);

    const created: string[] = [];
    const errors: EnvironmentCreationResult[] = [];
    ...
    for (const env of environments) {
        try {
            ...
            let reviewers: Array<{type: 'User' | 'Team'; id: number}> | undefined;

            if (env.reviewers && env.reviewers.length > 0) {
                reviewers = await Promise.all(
                    env.reviewers.map(async (reviewer: EnvironmentReviewer) => {
    ...
    for (const envVar of envVars) {
        ...
        for (const variable of envVar.variables) {
            try {
                ...
                const variableName = toUpperSnakeCase(variable.name);

                log.info(
                    `Setting variable ${variableName} for environment ${envVar.environmentName}`
                );
                ...
                log.info(
                    `Successfully set variable ${variableName} for environment ${envVar.environmentName}`
                );
                created++;
            } catch (error) {
                const errorMessage = error instanceof Error ? error.message : String(error);
                log.error(
                    `Failed to set variable ${variable.name} for environment ${envVar.environmentName}: ${errorMessage}`
                );
 ...
 */
const createRepositorySecrets = async (
    client: Octokit,
    log: Logger,
    owner: string,
    repo: string,
    ...
    created: number;
    errors: SecretCreationResult[];
}> => {
    log.info(`Creating repository secrets for ${owner}/${repo}`);

    let created = 0;
    const errors: SecretCreationResult[] = [];
 ...
 */
const productionalizeRepository = async (
    client: Octokit,
    log: Logger,
    owner: string,
    repo: string,
    ...
    const result: ProductionalizationResult = {
        teamPermissions: [],
        topicsAdded: false,
        environmentsCreated: [],
        environmentErrors: [],
    ...
    if (config.teamPermissions && config.teamPermissions.length > 0) {
        parallelTasks.push(
            (async () => {
                try {
                    result.teamPermissions = await addTeamPermissions(
                        client,
                        log,
                        owner,
                        repo,
            ...
            (async () => {
                try {
                    await createBranchProtection(
                        ...
                        owner,
                        repo,
                        config.branchProtectionPreset!,
                        config.branchProtectionTargetBranch
                    );
    ...
    if (config.environments && config.environments.length > 0) {
        try {
            const envResults = await createEnvironments(
                client,
                log,
                owner,
                repo,
            ...
            if (config.environmentVariables && config.environmentVariables.length > 0) {
                const varResults = await setEnvironmentVariables(
                    client,
                    log,
                    owner,
                    repo,
    ...
    if (config.secrets && config.secrets.length > 0) {
        try {
            const secretResults = await createRepositorySecrets(
                client,
                log,
                owner,
                repo,
 ...
 */
export const getProductionalizationService = (
    client: Octokit,
    log: Logger
): ProductionalizationService => {
    return {
        productionalizeRepository: (owner, repo, config) =>
            productionalizeRepository(client, log, owner, repo, config),
    };
};
